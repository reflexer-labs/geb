...
// Mocking an as simple as possible version of the safe, with just enough to allow LiquidationEngine to be tested without modifications
// Echidna breaks if setting up a full environment (like on unit tests)
// This contract will be setup from start with a single collateral, and allow only safe manipulation (1 safe)
contract SafeEngineMock {

  // --- Data ---
  // struct CollateralType { // fixed value per collateral
      // Total debt issued for this specific collateral type
      uint256 debtAmount = 10 ** 26;        // [wad], 100mm
      // Accumulator for interest accrued on this collateral type
      uint256 accumulatedRate = 10 ** 27;   // [ray]
      // Floor price at which a SAFE is allowed to generate debt
      uint256 safetyPrice = ray(1 ether);       // [ray]
      // Maximum amount of debt that can be generated with this collateral type
      uint256 debtCeiling = rad(1000 ether);       // [rad]
  ...
  // struct SAFE { // 1 safe
      ...
      uint256 lockedCollateral = 1000 ether;  // [wad]
      // Total amount of debt generated by a SAFE
      uint256 generatedDebt = 100 ether;     // [wad]
  // }
  ...
  // Helper functions to allow for fuzzing collateral settings and safe state
  ...
  function modifySafe(uint _lockedCollateral, uint _generatedDebt) public {
    lockedCollateral = _lockedCollateral;
    generatedDebt = _generatedDebt;
  }

  function denySAFEModification(address guy) public {}

  function approveSAFEModification(address guy) public {}

  function canModifySAFE(address safe, address guy) public view returns (bool) {
    return true;
  }

  function safes(bytes32 collateralType, address safe) public view returns (uint safeCollateral, uint safeDebt) {
    return (lockedCollateral,generatedDebt);
  }

  function collateralTypes(bytes32 collateralType) public view returns (uint, uint, uint, uint, uint, uint) {
    return (
        debtAmount,        // [wad]
        accumulatedRate,   // [ray]
        safetyPrice,       // [ray]
...
contract LiquidationEngineMock {
    ...
    **/
    ...
    CollateralType collateral = CollateralType(address(0), 1 ether, rad(10 ether));
    ...
    SafeEngineMock       public safeEngine;
    ...
    constructor(address safeEngine_) public {
        ...
        safeEngine = SafeEngineMock(safeEngine_);
    ...
    */
    ...
    function liquidateSAFE(bytes32 collateralType, address safe) external returns (uint256 auctionId) {
        ...
        if (both(liquidationPrice > 0, multiply(safeCollateral, liquidationPrice) < multiply(safeDebt, accumulatedRate))) {
          CollateralType memory collateralData = collateral; //collateralTypes[collateralType];
          ...
          assert(both(collateralToSell <= 2**255, limitAdjustedDebt <= 2**255)); //, "LiquidationEngine/collateral-or-debt-overflow");
