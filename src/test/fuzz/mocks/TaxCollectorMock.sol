...
contract TaxCollectorMock {
    ...
    **/
    ...
    function addition(uint256 x, uint256 y) internal pure returns (uint256 z) {
        z = x + y;
        assert(z >= x); //, "TaxCollector/add-uint-uint-overflow");
    }
    function addition(int256 x, int256 y) internal pure returns (int256 z) {
        z = x + y;
        if (y <= 0) assert(z <= x); //, "TaxCollector/add-int-int-underflow");
        if (y  > 0) assert(z > x); //, "TaxCollector/add-int-int-overflow");
    }
    function subtract(uint256 x, uint256 y) internal pure returns (uint256 z) {
        assert((z = x - y) <= x); //, "TaxCollector/sub-uint-uint-underflow");
    }
    function subtract(int256 x, int256 y) internal pure returns (int256 z) {
        z = x - y;
        assert(y <= 0 || z <= x); //, "TaxCollector/sub-int-int-underflow");
        assert(y >= 0 || z >= x); //, "TaxCollector/sub-int-int-overflow");
    }
    function deduct(uint256 x, uint256 y) internal pure returns (int256 z) {
        z = int256(x) - int256(y);
        assert(int256(x) >= 0 && int256(y) >= 0); //, "TaxCollector/ded-invalid-numbers");
    }
    function multiply(uint256 x, int256 y) internal pure returns (int256 z) {
        z = int256(x) * y;
        assert(int256(x) >= 0); //, "TaxCollector/mul-uint-int-invalid-x");
        assert(y == 0 || z / y == int256(x)); //, "TaxCollector/mul-uint-int-overflow");
    }
    function multiply(int256 x, int256 y) internal pure returns (int256 z) {
        assert(y == 0 || (z = x * y) / y == x); //, "TaxCollector/mul-int-int-invalid");
    }
    function rmultiply(uint256 x, uint256 y) internal pure returns (uint256 z) {
        z = x * y;
        assert(y == 0 || z / y == x); //, "TaxCollector/rmul-overflow");
        z = z / RAY;
    }
