...
contract PotTest is DSTest {
    ...
    function test_save_0d() public {
        assertEq(vat.good(self), rad(100 ether));

        pot.join(100 ether);
        assertEq(wad(vat.good(self)),   0 ether);
        assertEq(pot.pie(self),      100 ether);

        pot.drip();

        pot.exit(100 ether);
        assertEq(wad(vat.good(self)), 100 ether);
    }
    function test_save_1d() public {
        pot.join(100 ether);
        pot.file("sr", uint(1000000564701133626865910626));  // 5% / day
        hevm.warp(now + 1 days);
        pot.drip();
        assertEq(pot.pie(self), 100 ether);
        pot.exit(100 ether);
        assertEq(wad(vat.good(self)), 105 ether);
    }
    function test_drip_multi() public {
        ...
        hevm.warp(now + 1 days);
        pot.drip();
        assertEq(wad(vat.good(potb)),   105 ether);
        pot.file("sr", uint(1000001103127689513476993127));  // 10% / day
        hevm.warp(now + 1 days);
        pot.drip();
        assertEq(wad(vat.sin(vow)), 15.5 ether);
        assertEq(wad(vat.good(potb)), 115.5 ether);
    ...
    function test_save_multi() public {
        ...
        pot.drip();
        pot.exit(50 ether);
        assertEq(wad(vat.good(self)), 52.5 ether);
        ...
        pot.drip();
        pot.exit(50 ether);
        assertEq(wad(vat.good(self)), 110.25 ether);
    ...
    function test_fresh_chi() public {
        ...
        assertEq(pot.pie(self), 100 ether);
        pot.exit(100 ether);
        // if we exit in the same transaction we should not earn interest
        assertEq(wad(vat.good(self)), 100 ether);
    }
