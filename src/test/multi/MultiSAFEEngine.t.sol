...
//           sig, coinName, collateralType, safe, collateralSource, debtDestination, deltaCollateral, deltaDebt
//         );
//
...
//       bytes32 coinName, bytes32 collateralType, address safe, address collateralSrc, address debtDst, int deltaCollateral, int deltaDebt
//     ) public {
//         safeEngine.modifySAFECollateralization(coinName, collateralType, safe, collateralSrc, debtDst, deltaCollateral, deltaDebt);
//     }
...
//         (ok,) = address(safeEngine).call(abi.encodeWithSignature(sig, coinName, collateralType, self, self, self, collateralToDeposit, generatedDebt));
//     }
//
...
//         string memory sig = "transferSAFECollateralAndDebt(bytes32,bytes32,address,address,int256,int256)";
//         address self = address(this);
//         (ok,) = address(safeEngine).call(abi.encodeWithSignature(sig, coinName, collateralType, self, dst, deltaCollateral, deltaDebt));
//     }
//
...
//
//         safeEngine.addCollateralJoin("gold", address(this));
...
//           safeEngine.safes(coinName, collateralType, safe); generatedDebt_;
//         return lockedCollateral_;
//     }
//     function generatedDebt(bytes32 collateralType, address safe) internal view returns (uint) {
//         (uint lockedCollateral_, uint generatedDebt_) =
//           safeEngine.safes(coinName, collateralType, safe); lockedCollateral_;
...
//         assertEq(tokenCollateral("gold", address(this)), 1000 ether);
//         safeEngine.modifySAFECollateralization(coinName, "gold", me, me, me, 6 ether, 0);
...
//         safeEngine.modifyParameters(coinName, "gold", 'debtCeiling', rad(10 ether));
...
//         safeEngine.modifyParameters(coinName, "gold", 'debtCeiling', rad(5 ether));
...
//         safeEngine.modifySAFECollateralization(coinName, "gold", me, me, me, 10 ether, 5 ether); // safe draw
...
//
//         safeEngine.modifySAFECollateralization(coinName, "gold", me, me, me, 10 ether, 10 ether);
//         safeEngine.modifyParameters(coinName, "gold", 'safetyPrice', ray(0.5 ether));  // now unsafe
//
...
//         safeEngine.modifyParameters(coinName, "gold", 'safetyPrice', ray(0.4 ether));  // now unsafe
...
//         safeEngine.addAuthorization(coinName, c);
//
...
//
//         ali.modifySAFECollateralization(coinName, "gold", a, a, a, 10 ether, 5 ether);
//
//         // anyone can lock
//         assertTrue( ali.can_modifySAFECollateralization(coinName, "gold", a, a, a,  1 ether,  0 ether));
//         assertTrue( bob.can_modifySAFECollateralization(coinName, "gold", a, b, b,  1 ether,  0 ether));
//         assertTrue( che.can_modifySAFECollateralization(coinName, "gold", a, c, c,  1 ether,  0 ether));
//         // but only with their own tokenss
//         assertTrue(!ali.can_modifySAFECollateralization(coinName, "gold", a, b, a,  1 ether,  0 ether));
//         assertTrue(!bob.can_modifySAFECollateralization(coinName, "gold", a, c, b,  1 ether,  0 ether));
//         assertTrue(!che.can_modifySAFECollateralization(coinName, "gold", a, a, c,  1 ether,  0 ether));
//
...
//         assertTrue( ali.can_modifySAFECollateralization(coinName, "gold", a, b, a, -1 ether,  0 ether));
//         assertTrue( ali.can_modifySAFECollateralization(coinName, "gold", a, c, a, -1 ether,  0 ether));
//
//         // only the lad can draw
//         assertTrue( ali.can_modifySAFECollateralization(coinName, "gold", a, a, a,  0 ether,  1 ether));
//         assertTrue(!bob.can_modifySAFECollateralization(coinName, "gold", a, b, b,  0 ether,  1 ether));
//         assertTrue(!che.can_modifySAFECollateralization(coinName, "gold", a, c, c,  0 ether,  1 ether));
//         // the lad can draw to anywhere
//         assertTrue( ali.can_modifySAFECollateralization(coinName, "gold", a, a, b,  0 ether,  1 ether));
//         assertTrue( ali.can_modifySAFECollateralization(coinName, "gold", a, a, c,  0 ether,  1 ether));
//
//         safeEngine.mint(coinName, address(bob), 1 ether);
//         safeEngine.mint(coinName, address(che), 1 ether);
//
//         // anyone can wipe
//         assertTrue( ali.can_modifySAFECollateralization(coinName, "gold", a, a, a,  0 ether, -1 ether));
//         assertTrue( bob.can_modifySAFECollateralization(coinName, "gold", a, b, b,  0 ether, -1 ether));
...
//         assertTrue(!ali.can_modifySAFECollateralization(coinName, "gold", a, a, b,  0 ether, -1 ether));
//         assertTrue(!bob.can_modifySAFECollateralization(coinName, "gold", a, b, c,  0 ether, -1 ether));
//         assertTrue(!che.can_modifySAFECollateralization(coinName, "gold", a, c, a,  0 ether, -1 ether));
//     }
//
...
//         assertTrue(!che.can_modifySAFECollateralization(coinName, "gold", a, c, c,  0 ether,  1 ether));
//
//         ali.approveSAFEModification(coinName, address(bob));
//
...
//         safeEngine.modifyParameters(coinName, "gold", "debtFloor", rad(5 ether));
...
//       taxCollector.modifyParameters(coinName, "gold", "stabilityFee", 1000000564701133626865910626);  // 5% / day
//       safeEngine.addSystemComponent(address(taxCollector));
//
//       gold.approve(address(collateralA));
//       gold.approve(address(safeEngine));
...
//
//       safeEngine.modifyParameters(coinName, "gold", 'debtCeiling', rad(10 ether));
//       safeEngine.modifyParameters(coinName, 'safeDebtCeiling', 5 ether);
//
//       me = address(this);
//   }
//
...
//       assertEq(gold.balanceOf(address(collateralA)), 1000 ether);
//       assertEq(tokenCollateral("gold", address(this)), 1000 ether);
//       assertEq(safeEngine.safeDebtCeiling(coinName), 5 ether);
//   }
...
//       safeEngine.modifyCollateralBalance("gold", a, int(rad(20 ether)));
//
//       ali.modifySAFECollateralization(coinName, "gold", a, a, a, 10 ether, 7 ether);
//   }
//   function testFail_generate_debt_above_collateral_ceiling_but_below_safe_limit() public {
//       safeEngine.modifyParameters(coinName, "gold", 'debtCeiling', rad(4 ether));
...
//
//       ali.modifySAFECollateralization(coinName, "gold", a, a, a, 10 ether, 5 ether);
//       ali.modifySAFECollateralization(coinName, "gold", a, a, a, 0, -5 ether);
//   }
...
//       taxCollector.taxSingle(coinName, "gold");
//
//       ali.modifySAFECollateralization(coinName, "gold", a, a, a, 0, -4 ether);
//   }
...
//       ali.modifySAFECollateralization(coinName, "gold", a, a, a, 10 ether, 5 ether);
//
//       safeEngine.modifyParameters(coinName, 'safeDebtCeiling', 4 ether);
//
//       assertTrue(!try_modifySAFECollateralization("gold", 0, 2 ether));
//       ali.modifySAFECollateralization(coinName, "gold", a, a, a, 0, -1 ether);
//       assertTrue(!try_modifySAFECollateralization("gold", 0, 2 ether));
//
//       safeEngine.modifyParameters(coinName, 'safeDebtCeiling', uint(-1));
//       ali.modifySAFECollateralization(coinName, "gold", a, a, a, 0, 4 ether);
//   }
// }
//
...
//         safeEngine = new TestMultiSAFEEngine();
//         safeEngine.initializeCoin(coinName, uint(-1));
//         safeEngine.initializeCollateralType(coinName, "ETH");
//
...
//         safeEngine.addCollateralJoin("collateral", address(collateralA));
//         safeEngine.addCollateralJoin("collateral", address(this));
//
//         ethA = new ETHJoin(address(safeEngine), "ETH");
//         safeEngine.addCollateralJoin("ETH", address(ethA));
//         safeEngine.addCollateralJoin("ETH", address(this));
//
//         coin  = new DSDelegateToken("Coin", 'Coin');
//         coinA = new MultiCoinJoin(coinName, address(safeEngine), address(coin));
//         safeEngine.addAuthorization(coinName, address(coinA));
...
//         safeEngine.modifySAFECollateralization(coinName, collateralType, self, self, self, wad, coin_);
//     }
//     function try_disable_coin(address a) public payable returns (bool ok) {
//         string memory sig = "disableCoin(bytes32)";
//         (ok,) = a.call(abi.encodeWithSignature(sig, coinName));
//     }
...
//         address safe = address(this);
//         safeEngine.mint(coinName, address(this), 100 ether);
//         safeEngine.approveSAFEModification(coinName, address(coinA));
//         assertTrue( try_exit_coin(safe, 40 ether));
//         assertEq(coin.balanceOf(address(this)), 40 ether);
//         assertEq(safeEngine.coinBalance(coinName, me), rad(60 ether));
...
//         assertEq(safeEngine.coinBalance(coinName, me), rad(70 ether));
//     }
...
// }
//
// abstract contract MultiEnglishCollateralAuctionHouseLike {
//     struct Bid {
//         uint256 bidAmount;
...
//     BasicCollateralJoin collateralA;
//
//     MultiEnglishCollateralAuctionHouse collateralAuctionHouse;
//
//     address me;
//
//     bytes32 coinName     = "MAI";
...
//       bytes32 coinName, bytes32 collateralType, int lockedCollateral, int generatedDebt
...
//     }
//
//     function try_liquidate(bytes32 coinName, bytes32 collateralType, address safe) public returns (bool ok) {
...
//         safeEngine.modifyParameters(coinName, "globalDebtCeiling", rad(1000 ether));
//
//         collateralAuctionHouse = new MultiEnglishCollateralAuctionHouse(coinName, address(safeEngine), address(liquidationEngine), "gold");
//         collateralAuctionHouse.addAuthorization(address(liquidationEngine));
//
//         liquidationEngine.modifyParameters(coinName, "gold", "collateralAuctionHouse", address(collateralAuctionHouse));
//         liquidationEngine.modifyParameters(coinName, "gold", "liquidationPenalty", 1 ether);
//
//         safeEngine.addAuthorization(coinName, address(collateralAuctionHouse));
//
...
//         liquidationEngine.modifyParameters(coinName, "gold", "liquidationQuantity", rad(115792 ether));
//         (,,, uint256 liquidationQuantity) = liquidationEngine.collateralTypes(coinName, "gold");
//         assertEq(liquidationQuantity, rad(115792 ether));
//     }
//     function test_set_auction_system_coin_limit() public {
//         liquidationEngine.modifyParameters(coinName, "onAuctionSystemCoinLimit", rad(1));
//         assertEq(liquidationEngine.onAuctionSystemCoinLimit(coinName), rad(1));
//     }
//     function testFail_liquidation_quantity_too_large() public {
//         liquidationEngine.modifyParameters(coinName, "gold", "liquidationQuantity", uint256(-1) / 10 ** 27 + 1);
//     }
...
//         safeEngine.modifyParameters(coinName, "gold", 'safetyPrice', ray(205 ether));
//         safeEngine.modifyParameters(coinName, "gold", 'liquidationPrice', ray(205 ether));
//         safeEngine.modifySAFECollateralization(coinName, "gold", me, me, me, 1000 ether, 200000 ether);
//
...
//             bytes32(uint256(keccak256(abi.encode(coinName, bytes32("gold"), uint256(1)))) + 2),
...
//         safeEngine.modifyParameters(coinName, "gold", 'safetyPrice', ray(2.5 ether));
//         safeEngine.modifyParameters(coinName, "gold", 'liquidationPrice', ray(2.5 ether));
//
//         safeEngine.modifySAFECollateralization(coinName, "gold", me, me, me, 40 ether, 100 ether);
//
//         safeEngine.modifyParameters(coinName, "gold", 'safetyPrice', ray(2 ether));
//         safeEngine.modifyParameters(coinName, "gold", 'liquidationPrice', ray(2 ether));
//
//         liquidationEngine.modifyParameters(coinName, "gold", "liquidationQuantity", rad(111 ether));
//         liquidationEngine.modifyParameters(coinName, "gold", "liquidationPenalty", 1.1 ether);
//
//         uint auction = liquidationEngine.liquidateSAFE(coinName, "gold", address(this));
//         // the full SAFE is liquidated
//         (uint lockedCollateral, uint generatedDebt) = safeEngine.safes(coinName, "gold", address(this));
//         assertEq(lockedCollateral, 0);
//         assertEq(generatedDebt, 0);
//         // all debt goes to the accounting engine
//         assertEq(accountingEngine.totalQueuedDebt(coinName), rad(100 ether));
...
//         safeEngine.modifyParameters(coinName, "gold", 'liquidationPrice', ray(2 ether));
//
//         liquidationEngine.modifyParameters(coinName, "gold", "liquidationQuantity", rad(82.5 ether));
//         liquidationEngine.modifyParameters(coinName, "gold", "liquidationPenalty", 1.1 ether);
//
//         uint auction = liquidationEngine.liquidateSAFE(coinName, "gold", address(this));
...
//         // all debt goes to the accounting engine
//         assertEq(accountingEngine.totalQueuedDebt(coinName), rad(75 ether));
//         // auction is for all collateral
//         (,uint amountToSell,,,,,, uint256 amountToRaise) = collateralAuctionHouse.bids(auction);
...
//
//         liquidationEngine.modifyParameters(coinName, "gold", "liquidationQuantity", rad(200 ether));  // => liquidate everything
//         uint auction = liquidationEngine.liquidateSAFE(coinName, "gold", address(this));
//         (lockedCollateral, generatedDebt) = safeEngine.safes(coinName, "gold", address(this));
//         assertEq(lockedCollateral, 0);
//         assertEq(generatedDebt, 0);
//         assertEq(accountingEngine.debtQueue(coinName, now), rad(100 ether));
//         assertEq(safeEngine.tokenCollateral("gold", address(this)), 960 ether);
//     }
...
//         safeEngine.modifyParameters(coinName, "gold", 'liquidationPrice', ray(2.5 ether));
//
//         safeEngine.modifySAFECollateralization(coinName, "gold", me, me, me, 100 ether, 150 ether);
//
//         safeEngine.modifyParameters(coinName, "gold", 'safetyPrice', ray(1 ether));
//         safeEngine.modifyParameters(coinName, "gold", 'liquidationPrice', ray(1 ether));
//
//         (uint lockedCollateral, uint generatedDebt) = safeEngine.safes(coinName, "gold", address(this));
//         assertEq(lockedCollateral, 100 ether);
//         assertEq(generatedDebt, 150 ether);
//         assertEq(accountingEngine.unqueuedDebt(coinName), 0 ether);
//         assertEq(safeEngine.tokenCollateral("gold", address(this)), 900 ether);
//
//         liquidationEngine.modifyParameters(coinName, "onAuctionSystemCoinLimit", rad(75 ether));
//         liquidationEngine.modifyParameters(coinName, "gold", "liquidationQuantity", rad(100 ether));
//         assertEq(liquidationEngine.onAuctionSystemCoinLimit(coinName), rad(75 ether));
//         assertEq(liquidationEngine.currentOnAuctionSystemCoins(coinName), 0);
//         uint auction = liquidationEngine.liquidateSAFE(coinName, "gold", address(this));
//         assertEq(liquidationEngine.currentOnAuctionSystemCoins(coinName), rad(75 ether));
//         (lockedCollateral, generatedDebt) = safeEngine.safes(coinName, "gold", address(this));
//         assertEq(lockedCollateral, 50 ether);
//         assertEq(generatedDebt, 75 ether);
//         assertEq(accountingEngine.debtQueue(coinName, now), rad(75 ether));
//         assertEq(safeEngine.tokenCollateral("gold", address(this)), 900 ether);
//
//         assertEq(safeEngine.coinBalance(coinName, address(this)), rad(150 ether));
//         assertEq(safeEngine.coinBalance(coinName, address(accountingEngine)), 0 ether);
//     }
...
//         assertEq(liquidationEngine.onAuctionSystemCoinLimit(coinName), rad(75 ether));
//         assertEq(liquidationEngine.currentOnAuctionSystemCoins(coinName), 0);
//         liquidationEngine.liquidateSAFE(coinName, "gold", address(this));
//         assertEq(liquidationEngine.currentOnAuctionSystemCoins(coinName), rad(75 ether));
//
//         (lockedCollateral, generatedDebt) = safeEngine.safes(coinName, "gold", address(this));
//         assertEq(lockedCollateral, 50 ether);
...
//         safeEngine.modifyParameters(coinName, "gold", 'debtFloor', rad(150 ether));
//         safeEngine.modifyParameters(coinName, "gold", 'safetyPrice', ray(2.5 ether));
//         safeEngine.modifyParameters(coinName, "gold", 'liquidationPrice', ray(2.5 ether));
//
//         safeEngine.modifySAFECollateralization(coinName, "gold", me, me, me, 100 ether, 150 ether);
...
//         safeEngine.modifyParameters(coinName, "gold", 'liquidationPrice', ray(1 ether));
//
//         liquidationEngine.modifyParameters(coinName, "onAuctionSystemCoinLimit", rad(150 ether));
//         liquidationEngine.modifyParameters(coinName, "gold", "liquidationQuantity", rad(1 ether));
//
//         assertEq(liquidationEngine.onAuctionSystemCoinLimit(coinName), rad(150 ether));
//         assertEq(liquidationEngine.currentOnAuctionSystemCoins(coinName), 0);
//
//         assertEq(liquidationEngine.getLimitAdjustedDebtToCover(coinName, "gold", address(this)), 1 ether);
//
//         liquidationEngine.liquidateSAFE(coinName, "gold", address(this));
//     }
...
//         safeEngine.modifyParameters(coinName, "gold", 'liquidationPrice', ray(1 ether));
//
//         liquidationEngine.modifyParameters(coinName, "onAuctionSystemCoinLimit", rad(149 ether));
//         liquidationEngine.modifyParameters(coinName, "gold", "liquidationQuantity", rad(150 ether));
//
//         assertEq(liquidationEngine.onAuctionSystemCoinLimit(coinName), rad(149 ether));
//         assertEq(liquidationEngine.currentOnAuctionSystemCoins(coinName), 0);
//
//         assertEq(liquidationEngine.getLimitAdjustedDebtToCover(coinName, "gold", address(this)), 149 ether);
//
//         liquidationEngine.liquidateSAFE(coinName, "gold", address(this));
//     }
//     function test_liquidation_remaining_on_auction_limit_right_above_safe_debt() public {
//         safeEngine.modifyParameters(coinName, "gold", 'debtFloor', rad(149 ether));
//         safeEngine.modifyParameters(coinName, "gold", 'safetyPrice', ray(2.5 ether));
//         safeEngine.modifyParameters(coinName, "gold", 'liquidationPrice', ray(2.5 ether));
//
//         safeEngine.modifySAFECollateralization(coinName, "gold", me, me, me, 100 ether, 150 ether);
...
//         liquidationEngine.modifyParameters(coinName, "gold", "liquidationQuantity", rad(1000 ether));
//
//         assertEq(liquidationEngine.getLimitAdjustedDebtToCover(coinName, "gold", address(this)), 149 ether);
//
//         liquidationEngine.liquidateSAFE(coinName, "gold", address(this));
...
//         (, uint generatedDebt_) =
//           safeEngine.safes(coinName, collateralType, safe);
...
//         safeEngine.initializeCoin(coinName, uint(-1));
//         safeEngine.initializeCollateralType(coinName, "gold");
//         safeEngine.addSystemComponent(address(this));
//         safeEngine.addCollateralJoin("gold", address(this));
//
//         safeEngine.modifyParameters(coinName, "globalDebtCeiling", rad(100 ether));
//         safeEngine.modifyParameters(coinName, "gold", "debtCeiling", rad(100 ether));
//     }
//     function generateDebt(bytes32 collateralType, uint coin) internal {
//         safeEngine.modifyParameters(coinName, "globalDebtCeiling", rad(coin));
//         safeEngine.modifyParameters(coinName, collateralType, "debtCeiling", rad(coin));
//         safeEngine.modifyParameters(coinName, collateralType, "safetyPrice", 10 ** 27 * 10000 ether);
//
//         address self = address(this);
//         safeEngine.modifyCollateralBalance(collateralType, self,  10 ** 27 * 1 ether);
//         safeEngine.modifySAFECollateralization(coinName, collateralType, self, self, self, 1 ether, int(coin));
//     }
...
//         safeEngine.updateAccumulatedRate(coinName, "gold", ali, int(ray(0.05 ether)));
//         assertEq(totalAdjustedDebt("gold", self), rad(1.05 ether));
//         assertEq(safeEngine.coinBalance(coinName, ali), rad(0.05 ether));
//     }
// }
