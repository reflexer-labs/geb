...
contract MixedStratSurplusAuctionHouseMock {
    // --- Auth ---
    ...
    **/
    ...
    function addUint48(uint48 x, uint48 y) internal pure returns (uint48 z) {
        assert((z = x + y) >= x); // "MixedStratSurplusAuctionHouse/add-uint48-overflow");
    }
    function subtract(uint256 x, uint256 y) internal pure returns (uint256 z) {
        assert((z = x - y) <= x); // "MixedStratSurplusAuctionHouse/sub-underflow");
    }
    function multiply(uint256 x, uint256 y) internal pure returns (uint256 z) {
        assert(y == 0 || (z = x * y) / y == x); //  "MixedStratSurplusAuctionHouse/mul-overflow");
    }
     ...
     */
    function startAuction(uint256 amountToSell, uint256 initialBid) public isAuthorized returns (uint256 id) {
